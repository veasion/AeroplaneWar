...
public class Battleground implements Serializable {
	...
	public static final int[] TYPES = { TYPE_BLUE_STARS, TYPE_FIRE_MOUNT, TYPE_GREEN_MOUNT, TYPE_DESERT_STORM,
			TYPE_SKY_PILLAR, TYPE_ABYSS_BRIDGE, TYPE_CLOUD_LAYER, TYPE_STAR_SKY, TYPE_ROAD_VILLAGE };
	...
	public void draw(Graphics g) {
		...
		if (this.bgImages == null) {
			...
			g.drawImage(bgImage, 0, y >= 0 ? y - p.containerHeight : p.containerHeight - Math.abs(y), p.containerWidth,
					p.containerHeight, null);
		} else {
			// 背景数组循环算法
			g.drawImage(bgImages[index], 0, y, p.containerWidth, p.containerHeight, null);
			g.drawImage(bgImages[index < bgImages.length - 1 ? index + 1 : 0], 0,
					isUp ? p.containerHeight - Math.abs(y) : y - p.containerHeight, p.containerWidth, p.containerHeight,
					null);
		}
